...
class char_proxy {
	...
	MyString* s;
	//private constructor
	char_proxy(MyString* ss,std::size_t ii):
		s(ss),i(ii){}
public:
	operator char();
...
public:
	...
	char_proxy operator[](std::size_t);
...
char_proxy MyString::operator[](std::size_t pos) { return char_proxy(this, pos); }
...
char_proxy::operator char() { return *(s->begin() + i); }
void char_proxy::operator=(char c) {
	if (*s->shared_num != 1)
	{
		(s->shared_num)--;//注意顺序
		s->new_n_copy(s->_beg, s->_end);//出来时成员已指向新的内存
	}
	*(s->begin() + i) = c;
}